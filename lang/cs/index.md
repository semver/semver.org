...
Ve světě softwaru, jeho vývoje a správy existuje děsivé místo, kterému říkáme “peklo závislostí” (angl. dependancy hell). Čím vetší a komplexnější Váš projekt je, tím je větší šance, že se na tomto místě jednou ocitnete.
...
1. Software používající Sémantické verzování, MUSÍ mít nadefinované API, buď to přímo ve zdrojovém kódu a nebo v externí dokumentaci. V obou případech to musí být hlavně přesné a komplexní.
...
4. MAJOR verze s hodnotou 0 (0.y.z.) je určena pro počáteční vývoj. Cokoliv se může změnit a API v tomto formátu by NEMĚLO být považované za stabilní.
...
8. Číslo MAJOR (X) MUSÍ být zvýšené, když byly přidané změny, které způsobily zpětnou inkompatibilitu. MŮŽE zahrnout i změny v rámci MINOR a PATCH verze. Číslo MINOR i PATCH se MUSÍ vynulovat vždy, když se změní MAJOR verze.
...
10. Metadata MŮŽOU být označené ve verzi přidáním znaku plus (+) a sérií identifikátorů oddělených tečkou hned za číslo PATCH a nebo pomocí předběžné verze. Identifikátory MUSÍ obsahovat pouze ASCII alfanumerické znaky a pomlčku [0-9A-Za-z-], NESMÍ být prázdné a číselné identifikátory NESMÍ obsahovat úvodní nulu. Metadata by NEMĚLY hrát roli při volbě priority verze. Např.: verze 1.0.0-alpha+001, 1.0.0+20130313144700, 1.0.0-beta+exp.sha.5114f85 mají všechny stejnou prioritu.
...
Jako zodpovědní developeři určitě budete chtít kontrolovat, že všechno funguje podle Vašich předpokladů. Ve skutečnosti se jedná nepořádek a sématické verzování Vám umožní rozumný způsob, jak vydávat a aktualizovat knihovny tak, abyste nemuseli řešit nové verze závislostí, ušetřili čas a vyhli se zmatkům.

Pokud Vám tyto argumenty připadají rozumné, všechno co potřebujete udělat pro začátek používání Sémantického verzování je dát vědět, že to děláte, a následně ty pravidla také dodržovat. Odkazem ze souboru README z vaší aplikace/softwaru na tyto stránky dejte ostatním vědět, jaké používáte pravidla. Užitek na obou stranách.
