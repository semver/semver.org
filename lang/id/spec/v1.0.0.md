...
2. Versi normal HARUS ditulis dalam bentuk X.Y.Z, dengan X, Y, Z adalah bilangan bulat. X adalah versi *major*, Y adalah *minor*, dan Z adalah *patch*. Setiap elemen HARUS bertambah secara numerik dengan kenaikan sebesar satu. Contohnya: 1.9.0 -> 1.10.0 -> 1.11.0

3. Ketika nomor versi *major* bertambah, versi *minor* dan *patch* versi HARUS diatur ulang ke nol. Ketika nomor versi *minor* bertambah, nomor versi *patch* HARUS disetel ulang ke nol. Misalnya: 1.1.3 -> 2.0.0 dan 2.1.7 ->
2.2.0.

1. Setelah versi dirilis, isi dari versi tersebut TIDAK BOLEH dirubah. Setiap perubahan harus dirilis sebagai versi baru.

2. Versi *major* 0 (0.y.z) adalah untuk pengembangan awal. Apapun bisa bisa berubah kapan saja. API publik sebaiknya dianggap tidak stabil di versi ini.

3. Versi 1.0.0 adalah titik awal API publik. Cara nomor versi ini dinaikkan setelah rilis ini adalah tergantung dengan API publik ini dan bagaimana ia berubah.

4. Versi *patch* Z (x.y.Z | x > 0) HARUS dinaikkan jika ada perbaikan bug yang kompatibel dengan versi lama. Sebuah perbaikan bug didefinisikan sebagai perubahan internal yang memperbaiki perilaku internal.

5. Versi *minor* Y (x.Y.z | x > 0) HARUS dinaikkan jika ada fitur baru yang kompatibel dengan versi lama dalam API publik. Versi ini BISA dinaikkan jika ada tambahan fungsionalitas substansial atau terjadi peningkatan di dalam kode privat. Versi ini BISA diubah bersama dengan perubahan tingkat *patch*. Versi *patch* HARUS dikembalikan ke angka 0 jika versi *minor* dinaikkan.

6. Versi *major* X (X.y.z | X > 0) HARUS dinaikkan jika ada perubahan yang membuat versi baru
...
Subspesifikasi ini SEBAIKNYA digunakan jika Anda menggunakan sistem kontrol versi (Git, Mercurial, SVN, dll) untuk menyimpan kode Anda. Dengan menggunaakan sistem ini, alat otomatis  untuk memeriksa paket Anda dan menentukan kepatuhan SemVer dan versi yang dirilis.

1. SaatWhen tagging releases in a version control system, the tag for a version
