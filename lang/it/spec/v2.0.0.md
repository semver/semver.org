...
iniziale e poi incrementare la versione minore per ogni successivo rilascio.

### Come so quando rilasciare la 1.0.0?

Se il tuo software è usato in produzione, dovrebbe probabilmente già essere
alla 1.0.0. Se hai una API stabile sulla quale gli hanno iniziato a dipendere, 
dovresti essere alla 1.0.0. Se ti stai preoccupando molto relativamente alla
retrocompatibilità, dovresti probabilmente già essere alla 1.0.0.

### Questo non scoraggia lo sviluppo rapido e le iterazioni veloci?

La versione Major zero è tutta relativa allo sviluppo rapido. Se stai 
modificando l'API ogni giorno o dovresti essere già nella versione 0.y.z o
dovresti essere su un branch di sviluppo separato lavorando alla prossima
versione major.

### Se perfino la più piccola modifica non retrocompatibile all'API pubblica
comporta un colpo alla versione major, non finirei alla versione 42.0.0 
molto rapidamente?
...
tuo software, o nessuno sa con sicurezza quali metodi sono invocabili. A lungo 
andare, il Versionamento Semantico, e l'insistenza su una API pubblica ben 
documentata è in grado di far andare tutto liscio.

### Cosa faccio se rilascio accidentalmente una modifica non retrocompatibile
come versione minor?

Non appena realizzi di aver infranto le specifiche del Versionamento Semantico,
...
### Cosa dovrei fare se aggiorno le mie dipendenze senza modificare 
l'API pubblica?
...
### Cosa faccio se inavvertitamente altero l'API pubblica in un modo non 
conforme con la modifica al numero di versione (per esempio il codice 
introduce erroneamente una modifica che rompe la major in un rilascio di una 
patch).

Utilizza il buon senso. Se ha un platea immensa che sarebbe drasticamente
...
Ricorda, il Versionamento Semantico, è finalizzato alla comunicazione di 
...
### Il Versionamento Semantico ha un limite alla dimensione della stringa della 
versione?
