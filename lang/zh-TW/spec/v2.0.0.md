...
先行版號及編譯版號可以加到「主版號.次版號.修訂號」的後面，作為延伸。
...
何公共 API 的功能被標記為棄用時也「必須 `MUST`」遞增。也「可以 `MAY`」在內部私有
程式有大量新功能或改進被加入時遞增，其中「可以 `MAY`」包括修訂級別的改變。每當次
版號遞增時，修訂號「必須 `MUST`」歸零。
...
1. 編譯版號「可以 `MAY`」被標注在修訂版或先行版號之後，先加上一個連接號再加上一
連串以句點分隔的標識符號來修飾。標識符號「必須 `MUST`」由 ASCII 的英數字和連接
...
數值高低比較，有字母或連接號時則逐字以 ASCII 的排序來比較。數字的標識符號比非數
